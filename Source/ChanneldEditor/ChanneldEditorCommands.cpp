...
void FChanneldEditorCommands::RegisterCommands()
{
	...
	UI_COMMAND(ToggleCompatibleRecompilationCommand, "Enable Compatible Recompilation", "Turn on to enable recompilation and hot-reload after generating replication code", EUserInterfaceActionType::ToggleButton, FInputChord());
	...
	UI_COMMAND(RemoveDuplicateWorldSettingsCommand, "Remove Duplicate World Settings", "Remove duplicate world settings from the current level", EUserInterfaceActionType::Button, FInputChord());
}
