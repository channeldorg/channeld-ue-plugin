...
cd "{WorkDir}"
set jqPath="{JQPath}"
set podStatusJsonPath="{PodStatusJsonPath}"

set prevReason=null

:check

kubectl get pods -l {PodSelector} -o=jsonpath="{.items[*].status}" > %podStatusJsonPath%
...
if %isPodPhaseRunning% == 0 (
    if %isContainerRunning% == 0 (
        echo {PodDescriptionName} pod is running
        EXIT /B 0
    )
)
...
if %isPodPhasePending% == 0 (
    ...
    echo ERROR: {PodDescriptionName} pod is not deployed >&2
